 ...
 * Modified by Bill Capps on 2/8/2017
 */
public class ImageAnnotation {
    public static void main(String[] args) throws IOException {
        final ClarifaiClient client = new ClarifaiBuilder("3nRDgJFilrtxnT0FGerZ3I1izEujD1HNgxnJ9hwa", "ZG0eVoYy_lXjASGTssaBTZdSNDDkqtSorFlJw7lz")
        ...
        for (int i=0; i<files.length;i++){
            ...
            */
            summary = summary + "Scene " + (i+1) + " has the following elements: ";
            for (int j = 0; j < min(10,data.size()); j++) {
                summary = summary + data.get(j).name();
                if (j == 9 || j==data.size()-1 ) {
                    summary = summary + ".\n";
                } else {
                    summary = summary + ", ";
                }
